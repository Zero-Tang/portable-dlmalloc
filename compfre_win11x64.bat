...
cl .\port_win.c /I"%incpath%\ucrt" /I"%incpath%\shared" /I"%incpath%\um" /I"%ddkpath%\include" /Zi /nologo /W3 /WX /O2 /Zc:wchar_t /MT /FAcs /Fa"%objpath%\sample.cod" /Fo"%objpath%\sample.obj" /Fd"%objpath%\vc90.pdb" /GS- /Gy /TC /c /errorReport:queue

link "%objpath%\sample.obj" "%binpath%\dlmalloc.lib" /LIBPATH:"%libpath%\10.0.26100.0\um\x64" /LIBPATH:"%libpath%\10.0.26100.0\ucrt\x64" /LIBPATH:"%ddkpath%\lib\x64" /NOLOGO /INCREMENTAL:NO /OPT:REF /OPT:ICF /DEBUG /PDB:"%binpath%\sample.pdb" /OUT:"%binpath%\sample.exe" /SUBSYSTEM:CONSOLE /Machine:X64 /ERRORREPORT:QUEUE
